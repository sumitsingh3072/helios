...
export default function DashboardPage() {
    const { report, uploadedFileName } = useFinancialInsightsStore();
    ...
    // Generate spending breakdown data for chart
    const getSpendingBreakdown = () => {
        ...
        const savings = report.key_metrics.net_cash_flow.amount || 0;

        return [
            { name: 'Income', value: income },
            { name: 'Expenses', value: outflows - fees },
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <GlassCard className="p-5 bg-[#050A14] border-white/5">
                    <div className="flex items-center gap-3 mb-3">
                        <div className={cn(
                            ...
                            <PiggyBank className={cn(
                                "w-4 h-4",
                                getSavingsRate() >= 20 ? "text-green-400" : "text-yellow-400"
                            )} />
                        </div>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                        {getSavingsRate() >= 20 ? 'Healthy savings' : 'Consider saving more'}
                    </p>
                </GlassCard>
                ...
                <GlassCard className="p-5 bg-[#050A14] border-white/5">
                    ...
                    <p className="text-2xl font-bold text-orange-400">
                        {report.key_metrics.burn_rate.ratio_to_income || '0%'}
                    </p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <GlassCard variant="metallic" className="lg:col-span-2 p-0 overflow-hidden min-h-[400px] flex flex-col">
                    ...
                    <div className="flex-1 p-6 md:p-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center relative">
                        ...
                        <div className="relative z-10">
                            <p className="text-sm text-gray-400 mb-1">Statement Period Balance</p>
                            ...
                            <div className={cn(
                                ...
                                <span className="text-sm font-bold">
                                    {(report.key_metrics.net_cash_flow.amount ?? 0) >= 0 ? 'Positive' : 'Negative'} Cash Flow
                                </span>
                            </div>

                            <p className="text-xs text-gray-500 mt-4 max-w-xs">
                                {report.key_metrics.net_cash_flow.insight}
                            </p>
                        </div>
                        ...
                        <div className="relative z-10 space-y-4">
                            ...
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-sm text-green-400 font-bold">
                                        ₹{getTotalIncome().toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                                    </span>
                                </div>
                            ...
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className="text-sm text-orange-400 font-bold">
                                        ₹{(report.key_metrics.burn_rate.total_outflows || 0).toLocaleString('en-IN', { minimumFractionDigits: 0 })}
                                    </span>
                                </div>
                                <div className="h-1.5 w-full bg-black/50 rounded-full overflow-hidden">
                                    <div
                                        className="h-full bg-orange-500 shadow-[0_0_10px_#F97316]"
                                        style={{ width: `${Math.min(100, ((report.key_metrics.burn_rate.total_outflows || 0) / getTotalIncome()) * 100)}%` }}
                                    />
                                </div>
                            </div>
                            ...
                            <div className="p-4 rounded-xl bg-white/5 border border-white/5 backdrop-blur-sm">
                                <div className="flex justify-between mb-2">
                                    <span className="text-sm text-gray-400 font-medium">Banking Costs (Fees - Interest)</span>
                                ...
                                <div className="h-1.5 w-full bg-black/50 rounded-full overflow-hidden">
                                    <div
                                        ...
                                        style={{ width: `${Math.min(100, ((report.key_metrics.cost_of_funds.net_cost || 0) / getTotalIncome()) * 100)}%` }}
                                    />
                                </div>
                            </div>
                        </div>
                ...
                <GlassCard className="p-0 flex flex-col h-full">
                    ...
                    <div className="flex-1 overflow-auto p-4 space-y-3">
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-start gap-3">
                                        <span className={cn(
                                            "text-[10px] font-bold px-2 py-1 rounded border shrink-0 mt-0.5",
                                            colors.bg,
                                            colors.border,
                                            colors.text
                                        )}>
                        ...
                        <Link
                            ...
                            className="block text-center text-xs text-blue-400 hover:text-blue-300 py-2 transition-colors"
                        >
